...
for (const [name,url] of [['reportes-ventas','/admin/reportes/ventas'],['inventario','/admin/inventario/stock']]) {
  ...
  const info=await p.evaluate(()=>({
    ...
    filterOptions:[...document.querySelectorAll('.table-filter select option')].map(o=>o.textContent.trim()).slice(0,6),
  }));
