...
export class EstadosFinancierosService {
    ...
    balanceGeneral(anno: number) {
        return this.http.get<BalanceGeneral>(`${this.base}/balance-general/${anno}`).pipe(
            catchError(err => throwError(() => err))
        );
    }
    estadoResultados(anno: number) {
        return this.http.get<EstadoResultados>(`${this.base}/estado-resultados/${anno}`).pipe(
            catchError(err => throwError(() => err))
        );
    }
    flujoEfectivo(anno: number) {
        return this.http.get<FlujoEfectivo>(`${this.base}/flujo-efectivo/${anno}`).pipe(
            catchError(err => throwError(() => err))
        );
    }
}
